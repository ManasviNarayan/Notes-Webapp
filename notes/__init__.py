...
class Note():
    def create_note(self, notebook_id):
        new_note = {
                'title': 'New Note',
                'note': 'Take a note!',
...
class NoteBook(Note):
    def create_notebook(self, user, length):
        new_notebook = {
        ...
        Note.create_note(new_notebook['_id'])
        return new_notebook
...
class User(NoteBook):
    def create_user(self, ):
        ...
        NoteBook.create_notebook(new_user, length=0)
        return new_user
...
def new_notebook():
    ...
    notebook = NoteBook()
    new_notebook = notebook.create_notebook(user, length)
...
def new_note(notebook_id):
    note = Note()
    new_note = note.create_note(note) 
    return redirect(url_for('note', note_id=new_note['_id'],
